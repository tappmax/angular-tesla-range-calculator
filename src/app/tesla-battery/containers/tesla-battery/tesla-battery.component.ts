...
export class TeslaBatteryComponent implements OnInit {
    ...
    constructor(public formBuilder: FormBuilder, private batteryService: BatteryService) {}

    ngOnInit() {

        this.models = this.batteryService.getModelData();

        this.tesla = this.formBuilder.group({
    ...
    private calculateStats(models, value): Stat[] {
        return models.map(model => {
            ...
            const miles = this.models[model][wheels][climate ? 'on' : 'off']
